...
void command_activate() {
	ifstream mem_file("/systeminfo-files/stripts/systeminfo_script.sh");
	if(mem_file.good() == false) {
		system("cd /systeminfo-files && cat /sys/devices/system/cpu/cpu*/cpufreq/scaling_cur_freq >> systeminfo-cpu-status.txt");
	...
	else system("cd /systeminfo-files/scripts && ./systeminfo_remove_script.sh");
}
...
void command_remove() {
	...
	if(mem_file.good() == false) {
		...
		system("cd /systeminfo-files && rm systeminfo-cpu-status.txt >> systeminfo-errors.txt");
